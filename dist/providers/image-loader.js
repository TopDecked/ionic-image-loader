...
var ImageLoader = (function () {
    ...
    Object.defineProperty(ImageLoader.prototype, "isIonicWKWebView", {
        get: function () {
            return (this.platform.is('android') || this.isWKWebView) && (location.host === 'localhost:8080' || window.LiveReload);
        },
        enumerable: true,
        configurable: true
    });
    ...
    ImageLoader.prototype.createFileName = function (url) {
        ...
        var extension = url.replace(/.*(\.\w{3,4})($|.*\?.*$)/i, "$1");
        var filename = this.hashString(url).toString();
        var result = filename + (extension !== filename ? extension : "");
        return result;
    };
    /**
